...
def test_dynamic_parser():
    """Test the dynamic parser with the new PDF format"""
    pdf_path = "sample_autonomous.pdf"
    
    print("Testing dynamic autonomous parser...")
        ...
        if results:
            ...
            for i, student in enumerate(results[:3]):
                ...
                for j, subject in enumerate(student['subjectGrades'][:3]):
                    print(f"    {j+1}. {subject['code']}: {subject['subject']} -> {subject['grade']} (Credits: {subject['credits']})")
