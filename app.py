...
def placement_dashboard():
    try:
        ...
        if 'user_token' not in session:
            return redirect(url_for('admin_login'))
        return render_template('placement_dashboard_new.html')
    except Exception as e:
        logger.error(f"Error rendering placement dashboard: {e}")
        return jsonify({"error": "Failed to load placement dashboard"}), 500
...
@app.route('/api/placement/eligible-students', methods=['GET'])
def get_eligible_students():
    """Get placement eligible students (CGPA > 6.0, backlogs <= 4)"""
    try:
        if not FIREBASE_AVAILABLE or not db:
            return jsonify({"error": "Firebase not available"}), 503
        ...
        max_backlogs = int(request.args.get('max_backlogs', 4))
        class_range = request.args.get('class_range')  # e.g., "23B81A1266-23B81A12D0"
        ...
        # Calculate CGPA and filter eligible students
        eligible_students = []
        student_cgpa_map = {}
        
        # Group results by student to calculate overall CGPA
        for doc in docs:
            doc_data = doc.to_dict()
            ...
            if student_id and sgpa:
                if student_id not in student_cgpa_map:
                    student_cgpa_map[student_id] = {'sgpas': [], 'semesters': [], 'backlogs': 0}
                
                student_cgpa_map[student_id]['sgpas'].append(float(sgpa))
                student_cgpa_map[student_id]['semesters'].append(semester)
                
                # Count backlogs (subjects with grade F)
                subject_grades = doc_data.get('subjectGrades', [])
                for subject in subject_grades:
                    if subject.get('grade') == 'F':
                        student_cgpa_map[student_id]['backlogs'] += 1
        
        # Calculate CGPA and filter eligible students
        for student_id, data in student_cgpa_map.items():
            cgpa = sum(data['sgpas']) / len(data['sgpas']) if data['sgpas'] else 0
            backlogs = data['backlogs']
            
            # Apply filters
            if cgpa >= min_cgpa and backlogs <= max_backlogs:
                # Apply class range filter if provided
                if class_range and '-' in class_range:
                    start_range, end_range = class_range.split('-')
                    if not (start_range <= student_id <= end_range):
                        continue
                ...
                eligible_students.append({
                    ...
                    'cgpa': round(cgpa, 2),
                    'backlogs': backlogs,
                    'semesters_completed': len(data['semesters']),
                    'branch': student_id[6:8] if len(student_id) >= 8 else 'Unknown'  # Extract branch from ID
                })
        ...
        return jsonify({"error": f"Failed to get eligible students: {str(e)}"}), 500

@app.route('/api/placement/class-ranges', methods=['GET'])
def get_class_ranges():
    """Get available class ranges based on student IDs"""
    try:
        if not FIREBASE_AVAILABLE or not db:
            return jsonify({"error": "Firebase not available"}), 503
        ...
        # Group by batch and branch
        ranges = {}
        for student_id in student_ids:
            if len(student_id) >= 8:
                batch = student_id[:2]  # e.g., "23"
                branch = student_id[6:8]  # e.g., "A1"
                
                key = f"{batch}{branch}"
                if key not in ranges:
                    ranges[key] = {'min': student_id, 'max': student_id, 'count': 0}
                
                if student_id < ranges[key]['min']:
                    ranges[key]['min'] = student_id
                if student_id > ranges[key]['max']:
                    ranges[key]['max'] = student_id
                ranges[key]['count'] += 1
        ...
        class_ranges.sort(key=lambda x: x['range'])
        
        return jsonify({
            "class_ranges": class_ranges,
            "total_ranges": len(class_ranges)
        }), 200
        
    except Exception as e:
        logger.error(f"Error getting class ranges: {e}")
        return jsonify({"error": f"Failed to get class ranges: {str(e)}"}), 500
...
# -----------------------------------------------------------------------------
# Training & Placement Analysis API
# -----------------------------------------------------------------------------

@app.route('/api/placement/students-analysis')
